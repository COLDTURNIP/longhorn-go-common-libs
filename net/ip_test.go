...
func (h *testIPForPodHooks) interfaceAddrs(name string) ([]net.Addr, error) {
	...
	if !ok || (result.err == nil && result.addrs == nil) {
...
func TestGetInterfaceAddrsWithHooks(t *testing.T) {
	...
	t.Run("interface absent", func(t *testing.T) {
		...
		assert.EqualError(t, err, "interface lhnet1 not found: interface not found")
	})
...
func TestUnifiedPodIPSelection(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:   "missing PodIP returns specific error",
			family: IPFamilyUnspecified,
			hooks: &testIPForPodHooks{
				interfaces: map[string]testInterfaceResult{
					StorageNetworkInterface: {},
		...
		},
		{
			...
			expectedErr:     "no usable ipv6 address found on interface lhnet1 or no valid interface from Env Pod IP POD_IP",
			expectedNameUse: 0,
			expectedCalls:   []string{StorageNetworkInterface},
		},
		{
		...
		{
			name:   "no usable address returns invalid PodIP error",
			family: IPFamilyIPv4,
			hooks: &testIPForPodHooks{
				interfaces: map[string]testInterfaceResult{
					StorageNetworkInterface: {},
...
func TestGetIPForPodByNetworkAndFamilyFallbacks(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			expectedError: `invalid POD_IP "not-an-ip"`,
			expectedCalls: []string{StorageNetworkInterface},
		},
		{
			...
			expectedError:   "no usable ipv6 address found on interface lhnet1 or no valid interface from Env Pod IP POD_IP",
			expectedNameUse: 1,
...
func TestGetIPForPodByNetwork(t *testing.T) {
	const expectedStorageError = "no usable IP address found on interface lhnet1 or no valid interface from Env Pod IP POD_IP"
...
func TestPublicPodIPSelectors(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:    "deprecated selector wrapper",
			resolve: GetIPForPod,
		},
		{
...
func TestGetInterfaceNameByIPWithHooks(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			expectedErrorMessage:   "failed to list addresses for interface broken0: synthetic address lookup failure",
			expectedInterfaceCalls: 1,
