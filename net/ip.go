...
// getInterfaceAddrs returns the addresses for an interface. An absent
// interface returns no addresses and no error.
...
func getInterfaceAddrsWithHooks(name string,
	...
	for i := range interfaces {
		...
		if err != nil {
			return nil, errors.Wrapf(err, "interface %s doesn't have address", name)
		}
		if len(addrs) == 0 {
...
func SelectIPByNetworkPreference(storageNetworkPresent bool, storageIPs []string, podIPs []string) (string, error) {
	...
	for _, candidate := range candidates {
		if ip := net.ParseIP(candidate); ip != nil && ip.IsGlobalUnicast() {
...
// family, selected from the storage network before the primary pod interface.
